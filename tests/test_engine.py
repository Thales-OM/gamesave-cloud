...
class TestSnapshots:
    def test_snapshot_and_list(self, engine, save_dir):
        info = engine.snapshot("first")
        assert info.branch == "main"
        snaps = engine.list_snapshots()
        assert [s.id for s in snaps] == [info.id]

    def test_empty_snapshot_skipped(self, engine, save_dir):
    ...
    def test_restore_content(self, engine, save_dir):
        s1 = engine.snapshot("one")
        seed(save_dir, "slot1.sav", "v2")
        engine.snapshot("two")
        engine.restore(s1.id)
        assert (save_dir / "slot1.sav").read_text() == "v1"

    def test_restore_hard_moves_branch(self, engine, save_dir):
        s1 = engine.snapshot("one")
        seed(save_dir, "slot1.sav", "v2")
        engine.snapshot("two")
...
class TestBranches:
    def test_create_switch(self, engine, save_dir):
        ...
        exp_snap = engine.snapshot("on exp")
        assert exp_snap.branch == "exp"

    def test_switch_back_restores_files(self, engine, save_dir):
        seed(save_dir, "slot1.sav", "base")
    ...
    def test_duplicate_branch_rejected(self, engine, save_dir):
...
class TestBundleTransport:
    def test_export_import_roundtrip(
        self, engine, vault, game, tmp_path, save_dir
    ):
        seed(save_dir, "slot1.sav", "gen1")
        first = engine.snapshot("gen1")
        ...
        e2 = GitEngine(game=g2, repos_root=vault.parent / "vault-b")
