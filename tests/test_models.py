...
class TestGameEntry:
    def test_slug_deterministic_from_name(self, tmp_path):
        d1 = tmp_path / "a"
        d2 = tmp_path / "b"
        d1.mkdir()
        d2.mkdir()
    ...
    def test_slug_sanitizes(self, tmp_path):
        d = tmp_path / "x"
        d.mkdir()
    ...
    def test_default_branch_and_engine(self, tmp_path):
        d = tmp_path / "y"
        d.mkdir()
...
class TestMetadata:
    def test_add_and_find_game(self, tmp_path, save_dir):
        md = Metadata.load(path=tmp_path / "md.json")
    ...
    def test_duplicate_rejected(self, metadata, save_dir):
        with pytest.raises(MetadataError):
            metadata.add_game(GameEntry(name="My Game", path=save_dir))

    def test_find_missing_returns_none(self, metadata):
        assert metadata.find_game("nope") is None

    def test_roundtrip_persists_games(self, tmp_path, save_dir):
        path = tmp_path / "md.json"
        md = Metadata.load(path=path)
        md.add_game(GameEntry(name="Round Trip", path=save_dir))
        md.save()
        md2 = Metadata.load(path=path)
        assert len(md2.games) == 1
        assert md2.games[0].name == "Round Trip"

    def test_remotes_crud(self, tmp_path):
        md = Metadata.load(path=tmp_path / "md.json")
        r = RemoteConfig(
            name="usb", type="filesystem", options={"path": "D:/backups"}
        )
        md.add_remote(r)
